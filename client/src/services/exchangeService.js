...
export const initiateExchange = async (token, data) => {
  const res = await fetch(`${API_BASE}/exchange/initiate`, {
    method: 'POST',
    headers: {
...
export const confirmPayment = async (data) => {
  const res = await fetch(`${API_BASE}/exchange/confirm-payment`, {
    method: 'POST',
...
export const getMyExchanges = async (token) => {
  const res = await fetch(`${API_BASE}/exchange/my`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  if (!res.ok) throw new Error('Failed to fetch my exchanges');
  return res.json();
};

export const getTradeById = async (tradeId) => {
  const res = await fetch(`${API_BASE}/exchange/trade/${tradeId}`);
  if (!res.ok) throw new Error('Failed to fetch trade');
  return res.json();
};

export const getAdminStats = async (token) => {
  const res = await fetch(`${API_BASE}/exchange/admin/stats`, {
    headers: { Authorization: `Bearer ${token}` },
  });
...
export const getOperatorStatus = async () => {
  const res = await fetch(`${API_BASE}/exchange/operator`);
...
export const getLatestExchanges = async (limit = 10) => {
  const res = await fetch(`${API_BASE}/exchange/latest?limit=${limit}`);
  if (!res.ok) throw new Error('Failed to fetch latest exchanges');
  return res.json();
};
