...
export default function MyExchanges(){
  ...
  const getStatusIcon = (status) => {
    switch(status) {
      case 'PENDING': return <Clock className="text-blue-600" size={14} />
      case 'PAID': return <Activity className="text-blue-500 animate-pulse" size={14} />
  ...
  const getStatusColor = (status) => {
    switch(status) {
      ...
      case 'PAID': return 'text-blue-500 bg-blue-50 border-blue-200'
    ...
    <div className="min-h-screen bg-white text-gray-900 pb-10">
      <div className="max-w-6xl mx-auto px-4 md:px-6 py-6 md:py-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-black text-gray-900 italic uppercase tracking-tighter">My Settlements</h1>
            <p className="text-[10px] md:text-xs font-black text-gray-500 uppercase tracking-[0.2em] mt-1">Institutional Ledger Control</p>
          </div>
          <button
            onClick={fetchTrades}
            disabled={loading}
            className="flex items-center justify-center gap-2 px-5 py-2.5 bg-white border border-gray-200 text-blue-600 rounded-xl transition-all active:scale-95 text-[10px] font-black uppercase tracking-widest hover:border-blue-300 hover:bg-gray-50 shadow-sm"
          >
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-white border border-gray-200 rounded-[2.5rem] shadow-sm">
            <Activity size={48} className="mx-auto mb-4 text-gray-300" />
            <h3 className="text-xl font-black uppercase italic tracking-tighter mb-2 text-gray-900">No Records Found</h3>
            ...
            <button
              ...
              className="px-8 py-3 bg-blue-600 text-white font-black rounded-xl hover:bg-blue-700 transition-all active:scale-95 text-xs uppercase tracking-widest shadow-md shadow-blue-200"
            >
              Initiate New Trade
            </button>
          </div>
          ...
          <div className="space-y-4">
            {filteredTrades.map(trade => (
              <div
                key={trade._id}
                className="bg-white border border-gray-200 rounded-[2rem] p-5 md:p-8 hover:border-blue-300 hover:shadow-md transition-all cursor-pointer group relative overflow-hidden"
                onClick={() => navigate(`/trade/${trade._id}`)}
              >
                {/* Decorative Background ID */}
                <div className="absolute -right-4 -bottom-4 opacity-[0.02] text-8xl font-black italic pointer-events-none group-hover:opacity-[0.05] transition-opacity text-gray-900">
                   #{trade._id.slice(-4).toUpperCase()}
                </div>

                <div className="flex flex-wrap items-center justify-between mb-8 gap-4 relative z-10">
                  <div className="flex items-center gap-3">
                    <div className="text-[10px] font-mono font-bold text-gray-500 bg-gray-50 px-3 py-1 rounded-lg border border-gray-200 uppercase tracking-tighter">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative z-10">
                  <div className="border-l-2 border-red-200 pl-4">
                    ...
                    <p className="text-[9px] text-gray-400 font-bold uppercase mt-1 italic">{trade.sendMethod}</p>
                  </div>
                  <div className="border-l-2 border-green-200 pl-4">
                    ...
                    <p className="text-[9px] text-gray-400 font-bold uppercase mt-1 italic">{trade.receiveMethod}</p>
                  </div>
                  <div className="border-l-2 border-blue-200 pl-4">
                    <p className="text-[9px] text-gray-500 uppercase font-black mb-1 tracking-widest">Market Rate & Timestamp</p>
                    <p className="text-sm font-black text-gray-900 tracking-tight">1 USDT = ₹{trade.rate.toFixed(2)}</p>
                ...
                {trade.status === 'COMPLETED' && trade.txid && (
                  <div className="mt-8 p-4 bg-green-50 border border-green-200 rounded-2xl flex items-center justify-between">
                    <div>
                        <p className="text-[9px] text-green-600 font-black uppercase mb-1 tracking-widest">Blockchain Hash</p>
                        <p className="text-[10px] font-mono text-gray-600 break-all leading-tight max-w-[250px] md:max-w-md">{trade.txid}</p>
                    </div>
                ...
                {trade.status === 'CANCELLED' && trade.rejectionReason && (
                  <div className="mt-8 p-4 bg-red-50 border border-red-200 rounded-2xl">
                    ...
                    <p className="text-xs text-gray-600 italic font-medium">"{trade.rejectionReason}"</p>
                  </div>
                )}
              </div>
            ))}
        ...
        {trades.length > 0 && (
          <div className="mt-12 grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-white border border-gray-200 rounded-2xl p-5 text-center group hover:border-green-300 hover:shadow-md transition-all">
              <p className="text-2xl font-black text-green-600 tracking-tighter">{statusCounts.completed}</p>
              <p className="text-[9px] uppercase font-black text-gray-500 tracking-[0.2em] mt-1">Success</p>
            </div>
            <div className="bg-white border border-gray-200 rounded-2xl p-5 text-center group hover:border-blue-300 hover:shadow-md transition-all">
              <p className="text-2xl font-black text-blue-500 tracking-tighter">{statusCounts.paid}</p>
              <p className="text-[9px] uppercase font-black text-gray-500 tracking-[0.2em] mt-1">In Queue</p>
            </div>
            <div className="bg-white border border-gray-200 rounded-2xl p-5 text-center group hover:border-blue-300 hover:shadow-md transition-all">
              <p className="text-2xl font-black text-blue-600 tracking-tighter">{statusCounts.pending}</p>
              <p className="text-[9px] uppercase font-black text-gray-500 tracking-[0.2em] mt-1">Awaiting</p>
            </div>
            <div className="bg-white border border-gray-200 rounded-2xl p-5 text-center group hover:border-red-300 hover:shadow-md transition-all">
